...
class BookmarkView extends View {
  _parentElement = document.querySelector('.watchlist');
  _btnContainer = document.querySelector('.section__movieInfo');
  _watchListBtn = document.querySelector('.icon-watchlist');
  _errorMsg = ``;
  _ortherMsg = `Find a greate movie, start your collection!❤️`;
  _data;
  createMarkup() {
  ...
  addHandelBookmark(handler) {
    // toggling the bookmark display
    this._watchListBtn.addEventListener('click', function () {
      document
        .querySelector('.section__preview--header')
        .classList.toggle('watchlist--open');
    });
    this._btnContainer.addEventListener('click', function (e) {
      const addFavoriteBtn = e.target.closest('.bookmark-icon');
